...
docker exec -it postgres createdb -U postgres token_factory
docker-compose -f ./backend/dev.docker-compose.yml up --build --abort-on-container-exit --exit-code-from integration
